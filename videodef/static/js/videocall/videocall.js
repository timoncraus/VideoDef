...
let ws = new WebSocket(
    `ws://${window.location.host}/ws/videocall/${roomName}/`
);
const configuration = {
    ...
    navigator.mediaDevices
        ...
        .then((devices) => {
            const videoDevice = devices.find(
                (d) =>
                d.kind === "videoinput" && !d.label.toLowerCase().includes("droidcam")
            );
...
function updateButtonColors() {
    ...
    const micEnabled = localStream
        .getAudioTracks()
        .some((track) => track.enabled);
    const camEnabled = localStream
        .getVideoTracks()
        .some((track) => track.enabled);
