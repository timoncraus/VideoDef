    ...
    dummy_games = [
        {
            ...
            "description": "Интерактивная доска для совместного рисования \
                            и работы с изображениями в реальном времени. \
                            Поддерживает многопользовательский режим, \
                            перетаскивание и масштабирование картинок.",
        ...
        },
        {
            ...
            "description": "Увлекательная игра, которая поможет развить внимание, \
                            логику и пространственное восприятие. \
                            В этой игре вам предстоит собирать изображения, \
...
def my_games_view(request):
    ...
    puzzle_name_subquery = UserPuzzle.objects.filter(game_id=OuterRef("pk")).values(
        "name"
    )[:1]
    ...
    if sort_order_param == "desc":
        user_games_query = user_games_query.order_by(
            F(sort_field_db).desc(nulls_last=True)
        )
    else:
        user_games_query = user_games_query.order_by(
            F(sort_field_db).asc(nulls_last=True)
        )
    ...
    puzzle_details_map = {}
    if puzzle_game_pks:
        puzzles_data = UserPuzzle.objects.filter(game_id__in=puzzle_game_pks).only(
            "game_id", "preset_image_path", "user_image"
        )
...
def delete_game_view(request, game_id):
    ...
    try:
        ...
        if game_to_delete.genre and game_to_delete.genre.code == "PZL":
            try:
                if (
                    hasattr(game_to_delete, "puzzle_details")
                    and game_to_delete.puzzle_details
                ):
        ...
        return JsonResponse(
            {
                "status": "error",
                "message": "Игра не найдена или у вас нет прав на её удаление.",
            },
            status=404,
        )
    except Exception as e:
        print(
            f"Ошибка при удалении игры {game_id} для пользователя {request.user.username}: {e}"
        )
        traceback.print_exc()
        return JsonResponse(
            {"status": "error", "message": "Произошла ошибка при удалении игры."},
            status=500,
        )
...
def save_puzzle_view(request):
    ...
    try:
        ...
        if not name:
            return JsonResponse(
                {"status": "error", "message": "Название не может быть пустым."},
                status=400,
            )
        ...
        try:
            ...
            return JsonResponse(
                {
                    "status": "error",
                    "message": "Неверный или отсутствующий размер сетки.",
                },
                status=400,
            )
        ...
        try:
            ...
            if not isinstance(piece_positions, list) or not all(
                isinstance(p, int) for p in piece_positions
            ):
            ...
            if len(piece_positions) != expected_length:
                raise ValueError(
                    f"Количество позиций ({len(piece_positions)}) \
                                 не соответствует размеру сетки ({expected_length})."
                )

        except (TypeError, ValueError, json.JSONDecodeError) as e:
            print(
                f"Ошибка парсинга/валидации позиций: \
                    {e}, получено: '{piece_positions_str}'"
            )
            return JsonResponse(
                {
                    "status": "error",
                    "message": f"Неверный формат \
                             или содержимое позиций элементов: {e}",
                },
                status=400,
            )
        except KeyError:
            return JsonResponse(
                {
                    "status": "error",
                    "message": "Данные о позициях элементов не переданы.",
                },
                status=400,
            )
        ...
        if preset_path and uploaded_image_file:
            return JsonResponse(
                {
                    "status": "error",
                    "message": "Нельзя одновременно указать пресет и загрузить файл.",
                },
                status=400,
            )
        if not preset_path and not uploaded_image_file:
            return JsonResponse(
                {
                    "status": "error",
                    "message": "Необходимо выбрать пресет или загрузить изображение.",
                },
                status=400,
            )
        ...
        return JsonResponse(
            {"status": "success", "message": f'Пазл "{name}" успешно сохранен!'}
        )
        ...
        return JsonResponse(
            {"status": "error", "message": f"Ошибка введенных данных: {error_message}"},
            status=400,
        )
        ...
        if (
            trigger_error_text_part1 in db_error_message
            and trigger_error_text_part2 in db_error_message
        ):

            error_detail = f'Пазл с названием "{name}" и размером сетки {grid_size}x{grid_size} уже существует.'
            print(
                f"Ошибка уникальности пазла: {error_detail} | Оригинальная ошибка: {e}"
            )
            return JsonResponse(
                {"status": "error", "message": error_detail}, status=400
            )

        else:
            print(
                f"Непредвиденная ошибка целостности БД \
                  при сохранении пазла ({request.user.username}): \
        ...
        return JsonResponse(
            {
                ...
                "message": "Произошла внутренняя ошибка сервера \
                        при сохранении пазла. Попробуйте позже.",
            },
            status=500,
        )
...
def load_puzzles_view(request):
    ...
    try:
        ...
        print(
            "КРИТИЧЕСКАЯ ОШИБКА: \
              Жанр 'Пазл' (код PZL) не найден в базе данных!"
        )
        return JsonResponse({"status": "success", "puzzles": []})

    try:
        puzzles = (
            UserPuzzle.objects.filter(game__user=request.user, game__genre=puzzle_genre)
            .select_related("game")
        ...
        print(
            f"Ошибка при загрузке списка пазлов ({request.user.username}): {e.__class__.__name__}: {e}"
        )
        traceback.print_exc()
        return JsonResponse(
            {
...
def update_puzzle_view(request, game_id):
    ...
    """
    try:
        user_puzzle = get_object_or_404(
            UserPuzzle, game_id=game_id, game__user=request.user
        )
        ...
        try:
            ...
            print(
                f"Ошибка парсинга/валидации позиций при обновлении: \
                  {e}, получено: '{piece_positions_str}'"
            )
            return JsonResponse(
                {
                    "status": "error",
                    "message": f"Неверный формат или содержимое позиций элементов: {e}",
                },
                status=400,
            )
        ...
        if preset_path:  # Пользователь выбрал/оставил пресет
            ...
            if not current_has_preset and not current_has_user_image:
                return JsonResponse(
                    {
                        "status": "error",
                        "message": "Ошибка: изображение не было \
                                предоставлено для обновления.",
                    },
                    status=400,
                )
        ...
        return JsonResponse(
            {"status": "success", "message": f'Пазл "{name}" успешно обновлен!'}
        )
        ...
        return JsonResponse(
            {
                "status": "error",
                "message": "Пазл для обновления не найден \
                        или у вас нет прав на его изменение.",
            },
            status=404,
        )
        ...
        print(
            f"Непредвиденная ошибка при обновлении пазла \
              (ID: {game_id}, User: {request.user.username}): \
                {e.__class__.__name__}: {e}"
        )
        traceback.print_exc()
        return JsonResponse(
            {
                "status": "error",
                "message": "Произошла внутренняя ошибка сервера \
                при обновлении пазла. Попробуйте позже.",
            },
            status=500,
        )
