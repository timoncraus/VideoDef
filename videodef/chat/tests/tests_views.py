...
class ChatViewsTest(ChatTestBase):
    ...
    def test_get_chat_existing(self):
        ...
        response = self.client.get(
            reverse("chat:get_chat", args=[self.user1.unique_id, self.user2.unique_id])
        )
    ...
    def test_get_chat_new(self):
        ...
        response = self.client.get(
            reverse("chat:get_chat", args=[self.user1.unique_id, new_user.unique_id])
        )
    ...
    def test_get_chat_same_user_redirect(self):
        self.client.force_login(self.user1)
        response = self.client.get(
            reverse("chat:get_chat", args=[self.user1.unique_id, self.user1.unique_id])
        )
