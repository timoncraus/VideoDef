...
class DocumentViewsTest(DocumentTestBase):
    ...
    def test_delete_view(self):
        url = reverse("document:document_confirm_delete", kwargs={"pk": self.document.pk})
    ...
    def test_detail_view(self):
        url = reverse("document:public_document_detail", kwargs={"pk": self.document.pk})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Тестовый документ")
