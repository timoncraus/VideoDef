...
class ViewTests(TestCase):
    def setUp(self):
        ...
        self.user.profile = Profile.objects.create(
            ...
            role=self.role,
            gender=self.gender,
            max_search_distance=10,  # Добавляем значение по умолчанию
        )
        self.user.save()
    ...
    def test_register_view_post_valid(self):
        ...
        response = self.client.post(
            reverse("account:register"),
            {
                ...
                "patronymic": "Тестович",
                "date_birth": "2000-01-01",  # Исправлен формат
                "role": self.role.id,
                "gender": self.gender.id,
                "max_search_distance": 10,  # Добавляем обязательное поле
            },
            follow=True,
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_login_view_post_invalid(self):
        response = self.client.post(
            reverse("account:login"),
            {"identifier": "wronguser", "password": "wrongpass"},
        )
    ...
    def test_view_other_user(self):
        # Создаем второго пользователя для просмотра
        other_user = User.objects.create_user(
            username="otheruser",
            email="other@example.com",
            password="password123",
        )
        other_user.profile = Profile.objects.create(
            first_name="Другой",
            last_name="Пользователь",
            patronymic="Тестович",
            date_birth="2000-01-01",
        ...
        # Просматриваем профиль другого пользователя
        response = self.client.get(
            reverse("account:view_other_user", args=[other_user.unique_id])
        )
        self.assertEqual(response.status_code, 200)
